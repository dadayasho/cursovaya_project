    ...
    def check_file(self):
        ...
        with open(self.file, 'r') as file_txt:
            ...
            if re.findall(pattern, content):
                if re.findall(pattern_for_cvv, content) or re.findall(pattern_for_name, content) or (re.findall(pattern_for_cvv, content) and re.findall(pattern_for_name, content)):
                    return True
            else:
                return False
                ...
                for cell in row:
                    if not isinstance(cell, str):  # Check if cell contains text
                        ...
                        if pattern.search(cell):
                            if pattern_for_cvv.search(cell) or pattern_for_name.search(cell):
                                return "да"
                        else:
                            return "нет"
        return "нет"
            ...
            for element in root.iter():
                if not isinstance(element.text, str):  # Check if element text is a string
                    ...
                    if pattern.search(element.text):
                        if pattern_for_cvv.search(element.text) or pattern_for_name.search(element.text):
                            return "да"
                    else:
                        return "нет"
